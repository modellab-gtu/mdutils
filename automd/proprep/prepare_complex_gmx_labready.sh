...
Options:
  -p, --pdb ID             PDB ID (default: ${PROID})
  -l, --ligand RES         Ligand residue name, e.g. JZ4
...
prepare_protein() {
    ...
    log "Detecting protein molecule names from [ moleculetype ]"
...
EOF
    ...
    [[ -n "$LIG_MOLNAME" ]] || die "Could not detect ligand molecule name"
...
solvate_and_minimize() {
    ...
    gmx grompp -f "$EM_MDP" -c solv_ions.gro -p "$FINAL_TOP" -o em.tpr -maxwarn 1
